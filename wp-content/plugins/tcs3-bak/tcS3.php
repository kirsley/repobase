 ...
 * License: GPL2
 */
require('/usr/share/php/Aws/aws.phar');
...
class tcS3 {
	...
	public $networkActivated;
 	public $imgServer = "img.blogs.es";
// 	public $bucketFolder = "philipstv";///This NEEDS MODIFYING///
	...
	public function activate() {
		$options = array(
			...
			"access_secret" => "",
			"delete_after_push" => 1,
			"s3_url" => $this->options["s3_url"],
			"local_url" => "http://$this->imgServer/wp-content/",
	...
	public function add_images_rewrite() {
		...
		add_rewrite_rule('^$this->bucketFolder/(.+)$', 'index.php?file=$matches[1]', 'top');
	...
	public function build_attachment_url($url) {
		...
		$url = $protocol . "://" . $this->imgServer . "/" . $this->bucketFolder . "/wp-content" . $this->uploadDir . '/' . $matches[1];
		return $url;
	}
